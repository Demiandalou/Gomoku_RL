...
class Game(object):
    ...
    def __init__(self, board, **kwargs):
        ...
        color="#e4ce9f"
        self.colors=[[color]*9 for i in range(9)]
        self.whitex=[]
        self.whitey=[]
        self.blackx=[]
        self.blacky=[]

        self.mytable=plt.table(cellColours=self.colors,colWidths=[0.11 for i in range(10)],loc=(0, 0), cellLoc='center')
        plt.scatter(self.whitex,self.whitey,s=800,c='w')
        plt.scatter(self.blackx,self.blacky,s=800,c='black')
        self.mytable.scale(1,2.5)
        self.mytable.auto_set_font_size(False)
        self.mytable.set_fontsize(6)
        plt.title("Gomoku",pad=20)
        plt.xlim([0,9])
        plt.ylim([0,9])
        plt.axis('off')
        plt.show()
    ...
    def start_play(self, player1, player2, start_player=0, is_shown=1):
        ...
        while True:
            ...
            if current_player==1:
                # self.colors[7-x][y]="#ffffff"
                self.whitex.append(y+1)
                self.whitey.append(x+1)

            else:
                # self.colors[7-x][y]="#090909"
                self.blackx.append(y+1)
                self.blacky.append(x+1)
